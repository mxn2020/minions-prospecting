...
description: Agent skills for working with Minions Prospecting MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-prospecting data.
---

# Minions Prospecting Agent Skills

Skills for agents operating on the `minions-prospecting` toolbox.
...
```bash
# TypeScript
pnpm add @minions-prospecting/sdk
...
The `prospecting` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-prospecting/sdk/schemas';
...
```python
from minions_prospecting.schemas import custom_types
...
- This agent only writes to `minions-prospecting` — it reads from other toolboxes but never writes to them
